                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {HANDSHAKE_RESPONSES.map((r) => (
                    <form key={r} action={updateHandshakeResponseAction.bind(null, id, rev, h.id, r)}>
                      <button
                        type="submit"
                        className={`rounded-md px-2 py-1 text-xs font-medium ${h.response === r ? "bg-slate-900 text-white" : "border border-slate-300 text-slate-600 hover:bg-slate-50"}`}
                      >
                        {r}
                      </button>
                    </form>
            ...
            {candidateCount > 0 && (
              <form action={generateHandshakeItemsAction.bind(null, id, rev)}>
                <button type="submit" className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50">
                  Generate {candidateCount} new item(s) from current scope
                </button>
              </form>
            )}
          </CardBody>
        ...
        <Card>
          ...
          <CardBody className="space-y-2">
            ...
            <form action={addMeetingRequestAction.bind(null, id, rev)} className="flex gap-2">
              <input name="note" placeholder="Record a meeting request" className="flex-1 rounded-md border border-slate-300 px-2.5 py-1.5 text-sm" />
